...
- **"Average per sample":** a plain mean over the stored snapshots, not a time-weighted average. Snapshots are stored only on change (see Snapshots below), so a price that holds for months contributes one sample while a short sale that flips twice contributes two; the column can read as if sales were more common than they were. Fixing this needs a time-weighted average in `service.history`, deferred as a follow-up.
...
## API notes
- `GET /api/games?q=&limit=&offset=`: `limit` 1 to 100 (default 20), `offset` from 0 up to 2,147,483,647 (large enough for any real page; a bigger value is a validation error, not a query that reaches the database). The body is a list; the number of games matching `q` is in the `X-Total-Count` header.
...
- `POST /api/games/{id}/epic-check?region=US` looks up the Epic price if the game was not checked in the last `EPIC_COOLDOWN_HOURS`. It returns `{"status": ..., "checked_at": ...}` with status `fresh` (already checked recently, so no request was made — or, rarely, a request was made but lost a race with another request for the same game and is reported as if it had arrived just after the winner), `checked`, `busy` (over `EPIC_ON_DEMAND_PER_MINUTE` in this process) or `unavailable` (Epic rate limited us, so lookups pause for 15 minutes, or the lookup failed). A game Epic does not list still counts as checked, so it is not searched again within the cooldown. The limiter is per process: with several API workers each has its own limit.
...
- **Epic:** only seed games (`app/seed_games.json`) are refreshed daily. Other games get an Epic price when someone opens the game in the UI (`POST /api/games/{id}/epic-check`), or when an admin refreshes them (`POST /api/admin/games/{id}/refresh`).
